...
class Human(core.Agent):
    ...
    def __init__(self, a_id: int, rank: int, step_size: float):
    ...
    def save(self) -> Tuple:
        ...
        """
        return (self.uid, self.infected, self.infected_duration, self.step_size)
    ...
    def step(self):
        ...
        if alive:
            ...
            if not is_equal(min_ngh, pt.coordinates):
                direction = (min_ngh - pt.coordinates) * self.step_size
...
class Zombie(core.Agent):
    ...
    def __init__(self, a_id, rank, step_size: float):
        super().__init__(id=a_id, type=Zombie.TYPE, rank=rank)
        self.step_size = step_size

    def save(self):
        return (self.uid, self.step_size)
    ...
    def step(self):
        ...
        if not is_equal(max_ngh, pt.coordinates):
            direction = (max_ngh - pt.coordinates[0:3]) * self.step_size
...
def restore_agent(agent_data: Tuple):
    ...
    if uid[1] == Human.TYPE:
        if uid in agent_cache:
            ...
            h = Human(uid[0], uid[2], agent_data[3])
            agent_cache[uid] = h
        ...
        if uid in agent_cache:
            ...
            z = Zombie(uid[0], uid[2], agent_data[1])
            agent_cache[uid] = z
            return z
...
class Model:
    ...
    def __init__(self, comm, params):
        ...
        human_step_size = params['human_step_size']
        for i in range(pp_human_count):
            h = Human(i, self.rank, human_step_size)
            self.context.add(h)
        ...
        self.zombie_step_size = params['zombie_step_size']
        ...
        for i in range(pp_zombie_count):
            zo = Zombie(i, self.rank, self.zombie_step_size)
            self.context.add(zo)
            x = random.default_rng.uniform(local_bounds.xmin, local_bounds.xmin + local_bounds.xextent)
            y = random.default_rng.uniform(local_bounds.ymin, local_bounds.ymin + local_bounds.yextent)
            self.move(zo, x, y)
    ...
    def add_zombie(self, pt):
        z = Zombie(self.zombie_id, self.rank, self.zombie_step_size)
        self.zombie_id += 1
        self.context.add(z)
